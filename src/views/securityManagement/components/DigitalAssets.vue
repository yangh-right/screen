...
<script>
import { safetyGradeEvaluation } from '@/api/deviceManager';
...
export default {
  ...
  methods: {
    async initData() {
      let { resultData, status } = await safetyGradeEvaluation(this.waterPlantId);
      let colors = ['#FF5C5C', '#FF9145', '#FFDE00', '#00FFF7', '#00FFF7', '#00FFF7'];
      if (status === 'complete' && Array.isArray(resultData)) {
        let getmyd = [];
        let yData = [];
        if (resultData.length) {
          getmyd = resultData.map(r => r.score);
          yData = resultData.map(r => r.placeName);
        } else {
          getmyd = ['0', '0', '0'];
          yData = ['进水', '南池厌氧段', '提升泵房'];
        }
