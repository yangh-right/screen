...
<template>
  <unit-card cardTitle="门禁视频">
    <div class="card-inner">
      ...
      <div class="list">
        <!-- 遍历数据列表 -->
        <div class="list__item" v-for="item in dataList" :key="item.name" @click="goDetailManage(item)">
          <!-- 左侧区域：图标和总数 -->
          <div class="left">
            <div class="icon"></div>
            <div class="total">
          ...
          <div class="line">
            <div class="line__item online">
              <div class="title">
                <a-icon class="icon" type="ic_wifi"></a-icon>
...
<script>
// 导入API和mixin
import { securityAnalysis } from '@/api/mainPage';
  ...
  .list {
    ...
    &__item {
      ...
      &:nth-child(1) {
        .left {
          .icon {
            background: url('~@/assets/img/smartSafe/video_dark.png') no-repeat;
      ...
      &:nth-child(2) {
        .left {
          .icon {
            background: url('~@/assets/img/smartSafe/door_dark.png') no-repeat;
            background-size: 100% 100%;
          }
        }
      }
