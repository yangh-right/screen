...
<template>
  <div class="h5-player tw-flex tw-flex-col">
    <div :id="refId" v-resize="handleResize" class="tw-flex-1 tw-overflow-hidden aaa"></div>
    <div class="control-btns tw-flex tw-items-center tw-justify-between tw-text-white">
      <div class="tw-flex tw-items-center">
        <a-icon
          v-if="playType == '1' && !isPlaySuccess"
          type="ic_play_arrow"
          class="tw-mr-4 tw-ml-4"
          title="播放"
          @click="showPreview"
        ></a-icon>
        <a-icon
        ...
        <!-- <a-button type="link" size="small" :disabled="!isPlaySuccess" title="慢放" @click="Slow">
          <a-icon title="慢放" v-if="playType == '2'" type="ic_fast_rewind" class="tw-mr-4 tw-ml-4"></a-icon></a-button>
        <a-button type="link" size="small" :disabled="!isPlaySuccess" title="快放" @click="Fast">
          <a-icon title="快放" v-if="playType == '2'" type="ic_fast_forward" class="tw-mr-4"></a-icon>
        </a-button> -->
        <a-icon
          title="慢放"
          v-if="playType == '2'"
          ...
          class="tw-mr-4 tw-ml-4"
        ></a-icon>
        <a-icon title="快放" v-if="playType == '2'" type="ic_fast_forward" @click="Fast" class="tw-mr-4"></a-icon>

        <div class="tw-flex tw-w-12 tw-items-center tw-justify-center">
          <span v-if="playingRate < 0 && playingRate !== 1" class="tw-text-xs tw-text-yellow-300"
            >X {{ 1 / Math.abs(playingRate) }}
          </span>
          <span v-if="playingRate > 0 && playingRate !== 1" class="tw-text-xs tw-text-yellow-300"
            >X {{ playingRate }}
          </span>
      ...
      <div>
        ...
        <span v-if="isRecording" class="tw-mr-4 tw-text-red-600">
          Rec: {{ hour | addPrePadding }}:{{ minute | addPrePadding }}:{{ second | addPrePadding }}
        </span>
        <a-icon
          v-if="!isRecording"
          type="ic_videocam"
          class="tw-mr-4 tw-text-9"
          title="开始录像"
          @click="Record"
        ></a-icon>
        <a-icon
          ...
          class="tw-mr-4 tw-text-red-600"
          title="停止录像"
          @click="StopRecord"
        ></a-icon>
        <a-icon type="ic_camera_alt" class="tw-mr-4" title="抓图" @click="CapturePicture()"></a-icon>
        <a-icon type="ic_zoom_out_map" class="tw-mr-4" title="全屏" @click="FullSreen"></a-icon>
      </div>
    </div>
...
import Vue from 'vue';
import { getISCHlsNoParse, getPlaybackHlsNoParse, getYSISCVideoLive, getYSISCVideoBack } from '@/api/video';
  ...
  methods: {
      ...
      return this.player
        ?.JS_SetWindowControlCallback({
          ...
          async pluginErrorHandler(index, iErrorCode, oError) {
            ...
            if (['0x12f910011', '0x12f910023', '0x12f910027', '0x12f910029', '0x01900050'].includes(iErrorCode)) {
              await _this.Stop();
              // _this.play();
            }
          },
    ...
    async showPlayback(queryTime) {
      ...
      try {
        ...
        let res;
        if (this.type == '3') {
          res = await getPlaybackHlsNoParse({
