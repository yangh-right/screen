...
export default {
  ...
  data() {
    return {
      ...
      mapConfig: {
        zoom: 13,
        center: [120.19537828, 30.39179329],
        clusterOptions: {
          minPoints: 5,
          maxZoom: 14,
          radius: 50
  ...
  watch: {
    ...
    personLine: {
      ...
      handler(newPersonLine) {
        if (this.map) {
          if (newPersonLine.length) {
            this.addPersonLine();
          } else {
            // 清理之前的轨迹数据
            ['person-line', 'start-point', 'end-point', 'track-points', 'moving-point'].forEach(layerId => {
  ...
  methods: {
    ...
    handleResize() {
      ...
      this.resizeTimer = setTimeout(this.ignoreScale, 200);
    },
    ignoreScale() {
      try {
        ...
        const realScaleX = wScale ? Number((1 / wScale).toFixed(5)) : 1;
        const realScaleY = hScale ? Number((1 / hScale).toFixed(5)) : 1;

        Object.assign(mapContainer.style, {
          width: '1920px',
          height: '1200px',
          transformOrigin: 'left top',
          transform: `scale(${realScaleX}, ${realScaleY})`
        });

        this.map?.resize();
      } catch (error) {
        console.error('缩放调整失败:', error);
      }
    },
    ...
    startAnimation() {
      ...
      if (this.isPaused) {
        // 从暂停位置继续
        this.startTime = performance.now() - this.pausedProgress * this.duration;
        this.isPaused = false;
      } else {
        // 从头开始
    ...
    animate() {
      ...
      // 发射进度事件
      this.$emit('progress-change', progress * 100);
  ...
  created() {
    if (JSON.parse(storage.get('user_info'))) {
