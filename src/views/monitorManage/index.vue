 ...
 * @LastEditTime: 2024-06-17 15:45:05
...
<script>
import { getSafetyHelmetTokenUrl } from '@/api/mainPage';
...
export default {
  ...
  methods: {
    async getSafetyHelmet() {
      let parameter = {
        waterPlantId: '1663413717179568130',
        time: '2024',
        startTime: moment().startOf('year').format('YYYY-MM-DD HH:mm:ss')
      };
      let { resultData, status } = await getSafetyHelmetTokenUrl(parameter);
      if (status === 'complete') {
