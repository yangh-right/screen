  ...
  <unit-card :cardTitle="title">
    <div class="card-inner">
      ...
      <div class="points-list">
        <div v-for="(item, i) in pointsArr" :key="item.deviceId" class="point-tab">
          <div class="label">
            <div class="outer">
          ...
          <div class="value">
            {{ item.deviceValue }} <span class="unit">{{ item.unit }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  watch: {
    sourceTypeList: {
      ...
      handler(val) {
        if (val.length > 0) {
          this.pointsArr = [];
          let sourceTypeItem = val.filter(v => v.sourceTypeName.indexOf('电耗') > -1);
          if (sourceTypeItem.length > 0) {
            this.pointsArr = sourceTypeItem[0].carbonEmissionSourceDeviceAnalysisList;
            let option = {
              tooltip: {
                textStyle: {
                  color: '#FFF'
...
.card-inner {
  ...
  .echart-block {
    ...
    height: 156px;
    text-align: center;
    background: url('~@/assets/img/carbonEmission/pie_roportion2.png') no-repeat center;
