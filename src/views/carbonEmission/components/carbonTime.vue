 ...
 * @LastEditors: wangyr
 * @LastEditTime: 2024-07-30 17:22:39
 * @Description:
-->
<template>
    ...
    <div class="card-inner">
      <div class="chart-btns">
        <div
          v-for="item in tabs"
          :key="item.dictValue"
          :class="[currentPoint === item.dictValue ? 'active-btn' : '', 'point-btn']"
          size="small"
      ...
      <div v-if="pointsArr.length > 0" class="card-echart">
        <div v-for="item in pointsArr" :key="item.code" class="echart-block">
...
import { gasTimeData } from '@/api/carbonEmission.js';
...
export default {
  ...
  methods: {
    getLineDict() {
      sysDictListByCode('carbon_emission_source_type').then(res => {
        if (res.status === 'complete' && Array.isArray(res.resultData) && res.resultData.length > 0) {
          // 获取线段
          this.tabs = [
            {
              dictName: '全厂碳排放总览',
    ...
    async initData() {
      ...
      if (this.currentPoint != 'powerPv') {
        params['sourceTypeList'] = [this.currentPoint];
      }

      const { status, resultData } = await gasTimeData(params);
      if (status === 'complete') {
    ...
    initChart(dataTime) {
      let colors = [
        [
          {
            offset: 0,
            color: 'rgba(223,137,255, 0.5)'
          },
          {
            offset: 0.99,
            color: 'rgba(255,212,55, 0)'
          }
        ],
        [
          {
      ...
      let borderColors = ['#DF89FF', '#FFEC68'];
    ...
    handleChange(item) {
      this.currentPoint = item.dictValue;
      this.initData();
    }
  }
};
...
.card-inner {
  ...
  .chart-btns {
    ...
    .point-btn {
      width: 164px;
      height: 34px;
      background: url('~@/assets/img/carbonEmission/ac-point.png') no-repeat;
