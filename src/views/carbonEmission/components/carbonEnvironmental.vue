  ...
  watch: {
    sourceTypeList: {
      ...
      handler(val) {
        if (val.length > 0) {
          this.pointsArr = [];
          let sourceTypeItem = val.filter(v => v.sourceTypeName.indexOf('环境受纳') > -1);
...
.card-inner {
  ...
  .echart-block {
    ...
    height: 156px;
    text-align: center;
    background: url('~@/assets/img/carbonEmission/pie_roportion3.png') no-repeat center;
