 ...
 * @LastEditTime: 2024-09-10 19:13:25
 * @LastEditors: wangyr
 * @Description:故障分析
-->
<template>
    ...
    <div class="content-spin">
      <div class="card-inner">
        <div class="echart-block">
          <pie-chart :options="option"> </pie-chart>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async initData() {
      ...
      if (
        success &&
        Array.isArray(resultData?.depotOutMainCountDataList) &&
        resultData?.depotOutMainCountDataList.length > 0
      ) {
        this.pointsArr = resultData?.depotOutMainCountDataList.map(item => {
          return {
