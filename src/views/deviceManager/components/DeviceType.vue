...
<script>
import { drawPie } from '../model/chart';
import { deviceTypeLevelAnalysis } from '@/api/deviceManager.js';
...
export default {
  ...
  methods: {
    async initData() {
      const { status, resultData } = await deviceTypeLevelAnalysis(this.waterPlantId);
      if (status === 'complete') {
        resultData.deviceTypeAnalysisVOList.forEach(item => {
          this.pointsArr.push({
            name: item.name,
